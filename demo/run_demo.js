...
function speak(text) {
  ...
  const live = new Promise((r) => exec(`say -r 235 ${arg}`).on('close', r));
  const file = new Promise((r) => exec(`say -r 235 -o ${audioPath} ${arg}`).on('close', r));
    ...
    steps: [
      {
        ...
        database: DB_RAW,
        schema: SCHEMA_RAW,
        sql: `SELECT * FROM RAW_DATA.STOCK_PRICES where SYMBOL = 'AAPL' order by DATE desc;`,
      },
      {
        label: 'SEC Filings — RAW DATA',
        database: DB_RAW,
        schema: SCHEMA_RAW,
        sql: `SELECT COMPANY_NAME, FILING_DATE, SUBSTR(FILING_TEXT, 1, 200) FROM ${SCHEMA_RAW}.SEC_FILINGS;`,
      },
    ],
  },
  {
    ...
    steps: [
      {
        ...
        sql: `SELECT COMPANY_NAME, AI_SUMMARY FROM ${SCHEMA_CORTEX}.FILING_SUMMARIES;`,
      },
      {
        label: 'Sentiment Analysis — Cortex AI',
        database: DB_CORTEX,
        schema: SCHEMA_CORTEX,
        sql: `SELECT COMPANY_NAME, SENTIMENT, SENTIMENT_SCORE FROM ${SCHEMA_CORTEX}.FILING_SUMMARIES;`,
      },
      {
        label: 'Classification Breakdown — Cortex AI',
        database: DB_CORTEX,
        schema: SCHEMA_CORTEX,
        sql: `SELECT COMPANY_NAME, FORM_TYPE, SENTIMENT, CLASSIFICATION FROM CORTEX_AI.FILING_SUMMARIES;`,
      },
      {
      ...
      },
      {
        ...
        sql: `SELECT * FROM DASHBOARDS.STOCK_PREDICTION_DASHBOARD where SYMBOL = 'AAPL' order by DATE desc;`
      },
    ],
  },
  {
